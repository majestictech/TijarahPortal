...
helper::checkUserURLAccess('cat_manage','cat_edit');
?>
<!--breadcrumb-->
						...
						@csrf
                            <div class="col-md-6 ">
								...
								<div class="input-group"> <span class="input-group-text bg-transparent"><i class='bx bx-tag'></i></span>
									<input type="text" autofocus="autofocus" onfocus="this.setSelectionRange(this.value.length,this.value.length);" name="name" value="{{$categoryData->name}}" class="form-control border-start-0" id="storename" placeholder="{{ __('lang.entercategoryname')}}" required>
								</div>
							</div> 
							<div class="col-md-6 ">
								<label class="form-label">{{ __('lang.categoryname_ar')}} *</label>
								<div class="input-group"> <span class="input-group-text bg-transparent"><i class='bx bx-tag'></i></span>
									<input type="text" name="name_ar" value="{{$categoryData->name_ar}}" class="form-control border-start-0" placeholder="{{ __('lang.entercategoryname_ar')}}" required>
								</div>
							</div> 
							<!--
                            <div class="col-md-6 ">
								...
								<div class="input-group"> <span class="input-group-text bg-transparent"><i class='bx bx-store'></i></span>
									<input type="text" name="name_ur" value="{{$categoryData->name_ur}}" class="form-control border-start-0" placeholder="{{ __('lang.entercategoryname_ur')}}" required>
								</div>
							</div> 
							<div class="col-md-6 ">
								<label class="form-label">{{ __('lang.categoryname_ml')}} *</label>
								<div class="input-group"> <span class="input-group-text bg-transparent"><i class='bx bx-store'></i></span>
									<input type="text" name="name_ml" value="{{$categoryData->name_ml}}" class="form-control border-start-0" placeholder="{{ __('lang.entercategoryname_ml')}}" required>
								</div>
							</div>
 							<div class="col-md-6 ">
								<label class="form-label">{{ __('lang.categoryname_bn')}} *</label>
								<div class="input-group"> <span class="input-group-text bg-transparent"><i class='bx bx-store'></i></span>
									<input type="text" name="name_bn" value="{{$categoryData->name_bn}}" class="form-control border-start-0" placeholder="{{ __('lang.entercategoryname_bn')}}" required>
								</div>
							</div>
							-->
        						...
        						<div class="input-group">
        							...
        							<select name="storeType" class="form-select single-select" id="storeType" aria-label="Example select with button addon" required>
    									...
    									@foreach($storetype as $key=>$value)
    										<option value="{{$value->id}}" {{ ( $value->id == $categoryData->storeType) ? 'selected' : '' }} >{{$value->name}} </option>
    									@endforeach
        							</select>
        						</div>
        					</div>
