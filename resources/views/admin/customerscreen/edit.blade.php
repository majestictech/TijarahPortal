	...
	<div class="ps-1">
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb mb-0 p-0">
				...
				<li class="breadcrumb-item active" aria-current="page">{{ __('lang.editslider')}}</li>
			</ol>
		</nav>
	</div>
</div>
		...
		<div class="card border-top border-0 border-4 border-secondary">
			<div class="card-body p-5 ">
				<div class="card-title d-flex align-items-center">
					...
					<h5 class="mb-0 text-primary">{{ __('lang.editslider')}}</h5>
				</div>
				<hr>
				<form class="row g-3 pt-3" data-toggle="validator" method="post" action="{{route('customerscreen.update')}}" enctype="multipart/form-data">
						@csrf
                            
                            <div class="col-md-6">
                                <label class="form-label">{{ __('lang.image')}}</label>
								<div class="custom-file">
								<input type="file" name="customerScreenImage" data-error="{{ __('lang.req_image')}}" class="custom-file-input form-control">
							...
							<div class="col-12">				
								...
								<button type="submit" class="btn btn-secondary px-5">{{ __('lang.editslider')}}</button>
							</div>
