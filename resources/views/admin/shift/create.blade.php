	...
	<div class="ps-1">
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb mb-0 p-0">
				...
				<li class="breadcrumb-item active" aria-current="page"><i class="fadeIn animated bx bx-list-plus"></i> {{ __('lang.addshift')}}</li>
			</ol>
		</nav>
	</div>
</div>
		...
		<div class="card border-top border-0 border-4 border-secondary">
			<div class="card-body p-5 ">
				<div class="card-title d-flex align-items-center">
					...
					<h5 class="mb-0 text-primary">{{ __('lang.addshift')}}</h5>
				</div>
				<hr>
				<form class="row g-3 pt-3" method="post" action="{{route('shift.store')}}" data-toggle="validator">
				@if($errors->any())
				...
				@csrf
					<div class="col-md-12 ">
						...
						<div class="input-group"> <span class="input-group-text bg-transparent"><i class='bx bx-box'></i></span>
							<input type="text" autofocus="autofocus" name="title" class="form-control border-start-0" id="title" placeholder="{{ __('lang.title')}}" required>
						</div>
					</div>
					...
					<div class="col-6">
						...
						<div class="input-group"> <span class="input-group-text bg-transparent"><i class='bx bx-time' ></i></span>
							<input type="time" name="hoursOfServiceFrom" class="form-control border-start-0" id="from" required  />
						</div>
					</div>
					<div class="col-6">
						<label for="to" class="form-label">{{ __('lang.to')}}</label>
						<div class="input-group"> <span class="input-group-text bg-transparent"><i class='bx bx-time' ></i></span>
							<input type="time" name="hoursOfServiceTo" class="form-control border-start-0" id="to" required  />
						</div>
					</div>
					
					<div class="col-12">
					    <input type="hidden" name="storeId" value = "{{request()->route('id')}}" //>
						<button type="submit" class="btn btn-primary px-5">{{ __('lang.addshift')}}</button>
					</div>
				</form>
			</div>
		</div>		
