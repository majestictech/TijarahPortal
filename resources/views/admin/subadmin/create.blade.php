	...
	<div class="ps-1">
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb mb-0 p-0">
				...
				<li class="breadcrumb-item active" aria-current="page"><i class="fadeIn animated bx bx-list-plus"></i> {{ __('lang.addsub')}}</li>
			</ol>
		</nav>
	</div>
</div>
		...
		<div class="card border-top border-0 border-4 border-secondary">
			<div class="card-body p-5 ">
				<div class="card-title d-flex align-items-center">
					...
					<h5 class="mb-0 text-primary">{{ __('lang.addsub')}}</h5>
				</div>
				<hr>
				<form class="row g-3 pt-3" data-toggle="validator" method="post" action="{{route('subadmin.store')}}" >
				@if($errors->any())
				...
				@csrf
					...
					<div class="col-12">
						<button type="submit" class="btn btn-primary px-5">{{ __('lang.addsub')}}</button>
					</div>
				</form>
			</div>
		</div>		
