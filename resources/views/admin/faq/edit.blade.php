...
helper::checkUserURLAccess('faq_manage','faq_edit');
?>
	...
	<div class="ps-1">
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb mb-0 p-0">
				...
				<li class="breadcrumb-item active" aria-current="page"><i class="fadeIn animated bx bx-list-plus"></i> {{ __('lang.editFAQ')}}</li>
			</ol>
		</nav>
	</div>
</div>
		...
		<div class="card border-top border-0 border-4 border-secondary">
			<div class="card-body p-5 ">
				<div class="card-title d-flex align-items-center">
					...
					<h5 class="mb-0 text-primary">{{ __('lang.editFAQ')}}</h5>
				</div>
				<hr>
				<form class="row g-3 pt-3" method="post" action="{{route('faq.update')}}" data-toggle="validator">
				@csrf
					<div class="col-md-12 ">
						...
						<div class="input-group"> <span class="input-group-text bg-transparent"><i class='bx bx-help-circle'></i></span>
							<input type="text" autofocus="autofocus" onfocus="this.setSelectionRange(this.value.length,this.value.length);" name="question" class="form-control border-start-0" id="question" placeholder="{{ __('lang.enterquestion')}}" value="{{$FaqData->question}}" required />
						</div>
					</div>
					...
					<div class="col-md-12">
						...
						<div class="input-group"> <span class="input-group-text bg-transparent" style="align-items:flex-start !important;"><i class='bx bx-comment-detail' ></i></span>
							<textarea type="text" name="answer" class="form-control border-start-0" id="answer" placeholder="{{ __('lang.req_answer')}}"  required />{{$FaqData->answer}}</textarea>
						</div>
					</div>
					
					<div class="col-12">
						<input type="hidden" name="id" value = "{{$FaqData->id}}">
						<button type="submit" class="btn btn-primary px-5">{{ __('lang.editFAQ')}}</button>
