	...
	<div class="col-xl-12 mx-auto">
		<div class="card border-top border-0 border-4 border-secondary">
			<div class="card-body p-5 ">
				...
				<hr>
				<form class="row g-3 pt-3" data-toggle="validator" method="post" action="{{route('usersmanagement.store')}}" >
				@if($errors->any())
				...
				@csrf
					...
					<div class="col-md-6">
						...
						<div class="input-group"> <span class="input-group-text bg-transparent"><i class="fadeIn animated bx bx-user-circle"></i></span>
							<select value="" name="roleId" id="roleId" class="form-select single-select" required>  
								<option value="">{{ __('lang.selectuser')}}</option>
								
								@foreach($masRoles as $key=>$masRole)
									@if (old('roleId') == $masRole->id)
									<option value="{{$masRole->id}}" selected>{{$masRole->name}}</option>
									@else
